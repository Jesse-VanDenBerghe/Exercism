defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, entered_time_stamp} <- enter_building(now),
         {:ok, manual} <- find_counter_information(now),
         counter <- manual.(birthday),
         {:ok, checksum} <- stamp_form(entered_time_stamp, counter, form),
         pass_port_id <- get_new_passport_number(entered_time_stamp, counter, checksum) do
      {:ok, pass_port_id}
    else
